(function() {
			...
			'<div class="head"><span>{{{title}}}</span></div>' +					
				...
				'<div class="menu-items" style="height: 60%;max-height: unset;">' +
					...
					'{{#elements}}' +
						...
						//"nui://vorp_inventory/html/"..(l.meta.resim or v.image) ..".png"
							'{{#image}}<img style="width: 50px; height: 50px; background-blend-mode: screen; background-repeat: round; padding: 5px; float: center;" src="{{{image}}}">{{/image}}'+
							...
							'{{#isSlider}}<div class="arrows">'+
								...
								'<i class="fas fa-chevron-center"></i>'+
							'</div>{{/isSlider}}' +
						'</div>' +
				...
				//'<br>'+
				'<div style="display: flex;align-items: center;justify-content: space-evenly;flex-direction: column;align-content: center;margin:2px">'+
					'<div class="images" style="display: flex; flex-wrap: wrap; justify-content: flex-start; flex-direction: row; width: 100%;">'+
                    '{{#descriptionimages}}' +
                     '<div style="display: flex;flex-wrap: wrap;flex-direction: column;align-items: flex-end;margin: 10px">'+
                     '<span style="position: absolute;">{{{count}}}</span>'+
                    '<img style="width: 60px;height: 60px;background-image: url(nui://vorp_inventory/html/slot-bk.png);background-blend-mode: screen;background-repeat: round;align-content: center;padding: 1px;" src={{{src}}}>'+
                    '<span style="width: 100%; text-align: center; font-size: 11px;">{{{text}}}</span>'+
                    '</div>'+
                    '{{/descriptionimages}}'+
                    '</div>'+
	...
	;
    window.MenuData = {};
    MenuData.ResourceName = 'rs_crafting';
    MenuData.opened = {};
    MenuData.focus = [];
    MenuData.pos = {};
                ...
                for (let i = 0; i < menuData.elements.length; i++) {
                    ...
                    if(element.image){
						if(element.image.includes("https://")){
							...
							element.image = "nui://vorp_inventory/html/img/"+element.image;                    	
						}
						else{
							element.image = "nui://vorp_inventory/html/img/"+element.image+".png";
						}
                    }
                    switch (element.type) {
                        case 'default':
    ...
    window.onData = (data) => {
        switch (data.menubase_action) {

            case 'openMenu': {
                MenuData.open(data.menubase_namespace, data.menubase_name, data.menubase_data);
                break;
            }

            case 'closeMenu': {
                MenuData.close(data.menubase_namespace, data.menubase_name);
                break;
            }

            case 'controlPressed': {
                switch (data.menubase_control) {
